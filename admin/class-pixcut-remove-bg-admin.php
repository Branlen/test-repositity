...
class Pixcut_Remove_BG_Admin
{
	 ...
	 */
	private $plugin_name = 'pixcut-remove-bg';
	 ...
	 * @since    1.0.0
	 */
	public function enqueue_pixcut_styles()
	{
		wp_enqueue_style('wp-color-picker');
		wp_enqueue_style('pixcut-remove-bg-adminstyles', plugin_dir_url(__FILE__) . 'css/style.css');
	}
	 ...
	 * @since    1.0.0
	 */
	public function enqueue_pixcut_scripts()
	{

		wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/pixcut-remove-bg-admin.js', array('jquery'), $this->version, false);
	...
	{
		include plugin_dir_path(dirname(__FILE__)) . 'admin/partials/plugin-pixcut-remove-bg-display-admin.php';
	}

	public function Pixcut_Remove_BG_processing()
	{
		global $wpdb;
		...
		if (wp_verify_nonce(sanitize_text_field($_POST['_nonce']), 'update-options') && user_can(intval($_POST['schk']), 'edit_pages')) {
			...
			$sql     = "SELECT * FROM `" . $wpdb->prefix . "wc_pixcut_remove_bg`";
			$res     = $wpdb->get_results($sql);
			if ($process == 'start_queue' || $process == 'save') {
				update_option('Pixcut_RemoveBG_ApiKey', sanitize_text_field(trim($_POST['Pixcut_RemoveBG_ApiKey'])));
				update_option('Pixcut_RemoveBG_products', sanitize_text_field($_POST['Pixcut_RemoveBG_products']));
				update_option('Pixcut_RemoveBG_products_IDs', sanitize_text_field($_POST['Pixcut_RemoveBG_products_IDs']));
				update_option('Pixcut_RemoveBG_thumbnail', intval($_POST['Pixcut_RemoveBG_thumbnail']));
				update_option('Pixcut_RemoveBG_gallery', intval($_POST['Pixcut_RemoveBG_gallery']));
				update_option('Pixcut_RemoveBG_Background', sanitize_text_field($_POST['Pixcut_RemoveBG_Background']));
				update_option('Pixcut_RemoveBG_Background_Color', sanitize_hex_color($_POST['Pixcut_RemoveBG_Background_Color']));
				update_option('Pixcut_RemoveBG_Include_Processed', intval($_POST['Pixcut_RemoveBG_Include_Processed']));
			}
			if (empty(get_option('Pixcut_RemoveBG_ApiKey'))) {
				echo json_encode(array(
					'hasErrors' => true,
					'error_msg' => __('Missing API Key', 'Pixcut-remove-bg')
				));
				$this->start_log();
				$this->add_log(null, ['errors_msg' => [['title' => __('Missing API Key', 'pixcut-remove-bg')]]]);
				...
				try {
					$img_thumbnail = (1 == get_option('Pixcut_RemoveBG_thumbnail')) ? true : false;
					$img_gallery   = (1 == get_option('Pixcut_RemoveBG_gallery')) ? true : false;
					$this->start_log();
					if ('all' == get_option('Pixcut_RemoveBG_products')) {
						...
						if (!empty($postImages)) {
							...
							$background_image = '';
							if (!empty(sanitize_text_field($_POST['Pixcut_RemoveBG_Background']))) {
								if (sanitize_text_field($_POST['Pixcut_RemoveBG_Background']) == 'image') {
							...
							echo json_encode(array(
								...
								'processing' => false,
								'status'     => 'error',
								'error_msg'  => __('No images to process', 'pixcut-remove-bg')
							));
							$wpdb->update(
								$wpdb->prefix . "wc_pixcut_remove_bg",
								array(
							...
							$this->add_log(null, ['errors_msg' => [['title' => __('No images to process', 'pixcut-remove-bg')]]]);
							// if ( intval($_POST['Pixcut_RemoveBG_LastImage']) != 1 ) {
							$this->end_log();
							// }
							$this->deleteBackgroundImage();
						}
					} else {
						$IDs        = explode(',', preg_replace('/\s+/', '', get_option('Pixcut_RemoveBG_products_IDs')));
						$postImages = array();
						foreach ($IDs as $ID) {
							if (is_numeric($ID)) {
						...
						if (!empty($postImages)) {
							...
							if (!empty(sanitize_text_field($_POST['Pixcut_RemoveBG_Background']))) {
								if (sanitize_text_field($_POST['Pixcut_RemoveBG_Background']) == 'image') {
									...
									require_once ABSPATH . 'wp-admin/includes/file.php';
									require_once ABSPATH . 'wp-admin/includes/media.php';
									wp_delete_attachment(get_option('Pxcut_RemoveBG_Background_Image'), true);
									$attachment_id = media_handle_upload('Pixcut_RemoveBG_file', 0);
									update_option('Pixcut_RemoveBG_Background_Image', $attachment_id);
									$background_image = wp_get_attachment_image_url($attachment_id, 'medium');
								}
					...
					echo json_encode(array(
						...
						'processing' => false,
						'status'     => 'error',
						'error_msg'  => __('Pixcut Background removal error: ', 'pixcut-remove-bg') . $e->getMessage()
					));
				...
				if (!empty(intval($_POST['Pixcut_RemoveBG_NextPost']))) {
					$post_id       = get_post(intval($_POST['Pixcut_RemoveBG_NextPost']));
					$image_id      = trim(intval($_POST['Pixcut_RemoveBG_NextImage']));
					$img_thumbnail = trim(sanitize_text_field($_POST['Pixcut_RemoveBG_NextImageThumb']));
					$img_gallery   = trim(sanitize_text_field($_POST['Pixcut_RemoveBG_NextImageGallery']));
					$remove_bg_id  = trim(intval($_POST['Pixcut_RemoveBG_ID']));
					if (!empty($post_id) and !empty($image_id)) {
						try {
							$Remove_BG_api_key = get_option('Pixcut_RemoveBG_ApiKey');
							update_option('Pixcut_RemoveBG_Include_Processed', intval($_POST['Pixcut_RemoveBG_Include_Processed']));
							$wpdb->update(
								$wpdb->prefix . "wc_pixcut_remove_bg",
								array(
							...
							if ($image_processing === true) {
								...
								echo json_encode(array(
									...
									'processing' => false,
									'status'     => 'error',
									'error_msg'  => __('Error: ' . $image_processing, 'pixcut-remove-bg')
								));
								$wpdb->update(
									$wpdb->prefix . "wc_pixcut_remove_bg",
									array(
							...
							echo json_encode(array(
								...
								'processing' => false,
								'status'     => 'error',
								'error_msg'  => __('Background removal error: ', 'pixcut-remove-bg') . $e->getMessage()
							));
							$this->deleteBackgroundImage();
							if (intval($_POST['Pixcut_RemoveBG_LastImage']) != 1) {
								$this->end_log();
				...
				if (!empty(intval($_POST['Pixcut_RemoveBG_LastImage']))) {
					if (intval($_POST['Pixcut_RemoveBG_LastImage']) == 1) {
						$this->end_log();
						$wpdb->update(
							$wpdb->prefix . "wc_pixcut_remove_bg",
	...
	public function Pixcut_Remove_BG_admin_notice()
	{
		if ($_GET['page'] == 'pixcut_remove_bg') {
			include plugin_dir_path(dirname(__FILE__)) . 'admin/partials/plugin-pixcut-remove-bg-display-admin-notices.php';
		}
	}
		...
		if ($img_current_id = get_post_thumbnail_id($post_id)) {
			if ($img_id == $img_current_id) {
				...
				if (wp_delete_attachment($img_id) == false) {
					echo json_encode(array(
						'hasErrors' => true,
						'msg'       => __('File not deleted. Permission issue or file existence', 'pixcut-remove-bg')
					));
					wp_die();
				}
			}
	...
	protected function image_processing($post_id, $image_id, $Remove_BG_api_key, $img_thumbnail, $img_gallery, $remove_bg_id)
	{
		...
		if (is_array($image_info)) {
			if (get_option('Pixcut_RemoveBG_Include_Processed') or ($image_info['insert'] == true)) {
				$new_img_id = $this->remove_bg($image_id, $post_id, $image_info['full_path'], $image_info['insert'], $Remove_BG_api_key);
				// echo (json_encode($new_img_id));
	...
	protected function backupImages($img_id)
	{
		...
		try {
			...
			$sql = "SELECT * FROM `" . $wpdb->prefix . "wc_pixcut_remove_bg_backup` WHERE attach_id='$img_id'";
			$res = $wpdb->get_results($sql);
			if (empty($res[0])) {
				...
				if (!$fp = fopen($newFilePath, "wb")) {
					return __('Check uploads folder permissions', 'pixcut-remove-bg');
				};
				...
				if (fclose($fp) === FALSE) {
					return __('Check available disk space', 'pixcut-remove-bg');
				}
	...
	protected function remove_bg($old_img, $post_id, $filePATHFull, $newProccessRemoveBg, $Remove_BG_api_key)
	{
		...
		try {
			...
			if ($this->isJSON($response['body'])) {
				$errorResponseObj = json_decode($response['body']);
				$errorResponse = [];
				if ($errorResponseObj->Code != 200) {
					$errorResponse[] = ['title' => $errorResponseObj->Msg];
					return array(
						'errors'     => true,
						'code'       => 400,
						'img_id'     => $old_img,
						'errors_msg' => $errorResponse
					);
				}
			...
			if (!$fp = fopen($filePATHFull, "wb")) {
				$er['errors_msg'][0]['title'] = __('Check uploads folder permissions', 'pixcut-remove-bg');
				return $er;
			}
			...
			if (fclose($fp) === FALSE) {
				$er['errors_msg'][0]['title'] = __('Check available disk space', 'pixcut-remove-bg');
				return $er;
			}
			...
			if ($newProccessRemoveBg) {
				...
				$wpdb->insert(
					$wpdb->prefix . "wc_pixcut_remove_bg_backup",
					array(
						'attach_id'     => $attach_id,
	...
	public function Pixcut_Remove_BG_Restore_Backup()
	{
		global $wpdb;
		if (wp_verify_nonce(sanitize_text_field($_POST['_nonce']), 'update-options')  && user_can(intval($_POST['schk']), 'edit_pages')) {
			...
			if (empty($res[0])) {
				//buckup list
				$sql_backup = "SELECT * FROM `" . $wpdb->prefix . "wc_pixcut_remove_bg_backup`";
				...
				if (!empty($res_backup)) {
					...
					//clear table
					$sql = "TRUNCATE `" . $wpdb->prefix . "wc_pixcut_remove_bg_backup`";
					$wpdb->query($sql);
					echo json_encode(array(
						'hasErrors' => false,
						'msg'       => __('Pictures restored!', 'pixcut-remove-bg')
					));
				} else {
					echo json_encode(array(
						'hasErrors' => true,
						'msg'       => __('The recovery list is empty!', 'pixcut-remove-bg')
					));
				}
			} else {
				echo json_encode(array(
					'hasErrors' => true,
					'msg'       => __('Restore is not possible until processing is complete', 'pixcut-remove-bg')
				));
			}
		} else {
			echo json_encode(array(
	...
	public function Pixcut_Delete_backup()
	{
		global $wpdb;
		if (wp_verify_nonce(sanitize_text_field($_POST['_nonce']), 'update-options')  && user_can(intval($_POST['schk']), 'edit_pages')) {
			try {
				$sql = "SELECT * FROM " . $wpdb->prefix . "wc_pixcut_remove_bg_backup";
				$res = $wpdb->get_results($sql);
				if (!empty($res)) {
					foreach ($res as $item) {
						if (wp_delete_attachment($item->old_attach_id, true) == false) {
							echo json_encode(array(
								'hasErrors' => true,
								'msg'       => __('Write error. Check uploads folder permissions', 'pixcut-remove-bg')
							));
							wp_die();
						}
					}
					...
					echo json_encode(array(
						'hasErrors' => false,
						'msg'       => __('Successfully deleted', 'pixcut-remove-bg')
					));
				} else {
					echo json_encode(array(
						'hasErrors' => true,
						'msg'       => __('No pictures to delete!', 'pixcut-remove-bg')
					));
				}
			} catch (Exception $e) {
				echo json_encode(array(
					'hasErrors' => true,
					'msg'       => __('Delete error: ', 'pixcut-remove-bg') . $e->getMessage()
				));
			}
		} else {
			echo json_encode(array(
	...
	public function Pixcut_User_Aborted()
	{
		global $wpdb;
		if (wp_verify_nonce(sanitize_text_field($_POST['_nonce']), 'update-options')  && user_can(intval($_POST['schk']), 'edit_pages')) {
			$remove_bg_id = trim(intval($_POST['Pixcut_RemoveBG_ID']));
			try {
				...
				echo json_encode(array(
					'hasErrors' => false,
					'msg'       => __('Process aborted by user successfully', 'pixcut-remove-bg')
				));
			} catch (Exception $e) {
				echo json_encode(array(
					'hasErrors' => true,
					'msg'       => __('The process was interrupted but caused an error: ', 'pixcut-remove-bg') . $e->getMessage()
				));
			}
		} else {
			echo json_encode(array(
	...
	public function Pixcut_Preview_BG_Images()
	{
		if (wp_verify_nonce(sanitize_text_field($_POST['_nonce']), 'update-options')) {
			update_option('Pixcut_RemoveBG_ApiKey', sanitize_text_field(trim($_POST['Pixcut_RemoveBG_ApiKey'])));
			update_option('Pixcut_RemoveBG_Background', sanitize_text_field($_POST['Pixcut_RemoveBG_Background']));
			update_option('Pixcut_RemoveBG_Background_Color', sanitize_text_field($_POST['Pixcut_RemoveBG_Background_Color']));
			if (empty(get_option('Pixcut_RemoveBG_ApiKey'))) {
				echo json_encode(array(
					'hasErrors' => true,
					'msg'       => __('Missing API Key', 'pixcut-remove-bg')
				));
				$this->start_log();
				$this->add_log(null, ['errors_msg' => [['title' => __('Missing API Key', 'pixcut-remove-bg')]]]);
				wp_die();
			...
			if (!empty(sanitize_text_field($_POST['Pixcut_RemoveBG_Background']))) {
				...
				// 		echo json_encode( array(
				// 			'hasErrors' => true,
				// 			'msg'       => __( 'Write error. Check uploads folder permissions and/or available disk space', 'pixcut-remove-bg' )
				// 		) );
				// 		wp_die();
				// 	}
				// 	// update_option( 'Pixcut_RemoveBG_Background_Image', $attachment_id );
				...
				if (!empty($post_id)) {
					...
					if ($thumb_id > 0) {
						...
						if (is_wp_error($response)) {
							...
							echo json_encode(array(
								'hasErrors' => true,
								'msg'       => !empty($error_message) ? $error_message : __('Background removal error: ', 'pixcut-remove-bg') . $error_message
							));
						} else {
							echo json_encode(array(
								'hasErrors'   => false,
						...
						echo json_encode(array(
							'hasErrors' => true,
							'msg'       => __('Item has no thumbnail', 'pixcut-remove-bg')
						));
					}
				} else {
					echo json_encode(array(
						'hasErrors' => true,
						'msg'       => __('Enter post id', 'pixcut-remove-bg')
					));
				}
			} catch (Exception $e) {
				echo json_encode(array(
	...
	protected function error_handling($param, $remove_bg_id)
	{
		...
		if ($param['code'] != '400') {
			...
			$wp_upload_dir = wp_upload_dir();
			$log_file_dir  = $wp_upload_dir['basedir'] . '/pixcut-remove-bg-log';
			if (!file_exists($log_file_dir)) {
				mkdir($log_file_dir, 0755, true);
			}
			$row = "[" . date('Y-m-d H:i:s') . "] " . __('Background removal error: ', 'pixcut-remove-bg') . $errors_msg . "\n";
	...
	protected function start_log()
	{
		...
		$row .= get_option('Pixcut_RemoveBG_Background') . "\n";
		if (get_option('Pixcut_RemoveBG_Background') == 'color') {
			$row .= "color: " . get_option('Pixcut_RemoveBG_Background_Color');
			$row .= "\n";
		} elseif (get_option('Pixcut_RemoveBG_Background') == 'image') {
			$row .= "color: " . wp_get_attachment_image_url(get_option('Pixcut_RemoveBG_Background_Image'), 'medium');
			$row .= "\n";
		}
		$row .= "\n";
	...
	protected function process_log()
	{
		...
		$row = "Processed " . intval($_POST['Pixcut_RemoveBG_CountProcessImage']) . " images of " . intval($_POST['Pixcut_RemoveBG_AllCountImage']) . "\n";
		$fp  = fopen($log_file_dir . '/log.txt', "a");
		fwrite($fp, $row);
		fclose($fp);
	}
